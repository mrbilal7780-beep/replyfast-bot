...
} from '\var\task\Documents\replyfast/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '\var\task\Documents\replyfast/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('\var\task\Documents\replyfast')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '\var\task\Documents\replyfast',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('\var\task\Documents\replyfast\.netlify\dist\run\handlers\server.js')
