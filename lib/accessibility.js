...
export class VoiceReader {
  constructor(options = {}) {
    ...
    // Optimisation mobile : réglages adaptés pour meilleure qualité
    if (this.isMobile) {
      this.rate = options.rate || 0.9; // Plus lent sur mobile pour clarté
      this.pitch = options.pitch || 1.1; // Pitch légèrement plus haut
  ...
  loadVoices() {
    ...
    // OPTIMISATION MOBILE: Préférer voix locales de haute qualité
    if (this.isMobile) {
      // Priorité 1: Voix locale française de haute qualité
      const localFrVoice = voices.find(v =>
        v.lang.startsWith('fr') &&
        v.localService &&
        (v.name.includes('Thomas') || v.name.includes('Amelie') || v.name.includes('enhanced'))
      );

      // Priorité 2: N'importe quelle voix locale française
      const anyLocalFrVoice = voices.find(v => v.lang.startsWith('fr') && v.localService);

      // Priorité 3: N'importe quelle voix française
      const anyFrVoice = voices.find(v => v.lang.startsWith('fr'));

      this.voice = localFrVoice || anyLocalFrVoice || anyFrVoice || voices[0];

      console.log('🎤 Mobile voice selected:', this.voice?.name, '| Local:', this.voice?.localService);
    } else {
      // Desktop: voix par défaut
  ...
  speak(text, options = {}) {
    ...
    // OPTIMISATION MOBILE: Chunker le texte en morceaux plus petits
    // Mobile Speech API peut couper les longs textes
  ...
  chunkText(text, maxLength) {
    // Découper le texte intelligemment par phrases
