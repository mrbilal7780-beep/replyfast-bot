...
export default function Onboarding() {
  ...
  const handleWhatsAppConnect = async () => {
    ...
    try {
      ...
      setWahaError(`⚠️ Impossible de générer le QR code. ${error.message.includes('Unauthorized') ? 'Le serveur WhatsApp n\'est pas configuré.' : error.message} Vous pouvez configurer WhatsApp plus tard.`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchQRCode = async (session) => {
    try {
      ...
      } else if (data.waiting && data.status === 'STOPPED') {
        // Session arrêtée
        setWahaError('🔄 Session WhatsApp arrêtée. Cliquez à nouveau sur "Générer le QR Code" pour la redémarrer.');
      }
    } catch (error) {
